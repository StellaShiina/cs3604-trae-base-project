...
WHERE t.train_no IN ('D5','G1','G2','D6','K1')
AND NOT EXISTS (
...
WHERE ts.train_no='D5' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
...
WHERE ts.train_no='D5' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date AND NOT EXISTS(
...
WHERE ts.train_no='D5' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date
...
WHERE ts.train_no='G1' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
...
FROM train_services ts JOIN stations s1 ON s1.code='BJP' JOIN stations s2 ON s2.code='SHH'
WHERE ts.train_no='G1' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date AND NOT EXISTS(
  SELECT 1 FROM service_segments seg WHERE seg.train_service_id=ts.id AND seg.from_stop_seq=1 AND seg.to_stop_seq=2);
INSERT INTO segment_seat_inventory(train_service_id,segment_id,seat_type,total_seats,left_seats,price_cents)
SELECT ts.id, seg.id, x.seat_type, x.total, x.left_qty, x.price
FROM train_services ts
...
JOIN (VALUES
  ...
  ('preferredFirst'::seat_type_enum,40,15,103000),
  ('business'::seat_type_enum,50,25,180000)
) AS x(seat_type,total,left_qty,price) ON true
WHERE ts.train_no='G1' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date
ON CONFLICT (train_service_id,segment_id,seat_type) DO UPDATE SET total_seats=EXCLUDED.total_seats,left_seats=EXCLUDED.left_seats,price_cents=EXCLUDED.price_cents,currency='CNY';

-- G2 08:00-12:20
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
SELECT ts.id, s1.id, 1, NULL, '08:00'::time FROM train_services ts JOIN stations s1 ON s1.code='BJP'
WHERE ts.train_no='G2' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
...
FROM train_services ts JOIN stations s1 ON s1.code='BJP' JOIN stations s2 ON s2.code='SHH'
WHERE ts.train_no='G2' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date AND NOT EXISTS(
  SELECT 1 FROM service_segments seg WHERE seg.train_service_id=ts.id AND seg.from_stop_seq=1 AND seg.to_stop_seq=2);
INSERT INTO segment_seat_inventory(train_service_id,segment_id,seat_type,total_seats,left_seats,price_cents)
SELECT ts.id, seg.id, x.seat_type, x.total, x.left_qty, x.price
FROM train_services ts
...
WHERE ts.train_no='G2' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date
ON CONFLICT (train_service_id,segment_id,seat_type) DO UPDATE SET total_seats=EXCLUDED.total_seats,left_seats=EXCLUDED.left_seats,price_cents=EXCLUDED.price_cents,currency='CNY';

-- D6 09:00-11:12（更快的 D 字车）
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
SELECT ts.id, s1.id, 1, NULL, '09:00'::time FROM train_services ts JOIN stations s1 ON s1.code='BJP'
WHERE ts.train_no='D6' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
...
FROM train_services ts JOIN stations s1 ON s1.code='BJP' JOIN stations s2 ON s2.code='SHH'
WHERE ts.train_no='D6' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date AND NOT EXISTS(
  SELECT 1 FROM service_segments seg WHERE seg.train_service_id=ts.id AND seg.from_stop_seq=1 AND seg.to_stop_seq=2);
INSERT INTO segment_seat_inventory(train_service_id,segment_id,seat_type,total_seats,left_seats,price_cents)
SELECT ts.id, seg.id, x.seat_type, x.total, x.left_qty, x.price
FROM train_services ts
...
WHERE ts.train_no='D6' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date
ON CONFLICT (train_service_id,segment_id,seat_type) DO UPDATE SET total_seats=EXCLUDED.total_seats,left_seats=EXCLUDED.left_seats,price_cents=EXCLUDED.price_cents,currency='CNY';

INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
SELECT ts.id, s1.id, 1, NULL, '19:00'::time FROM train_services ts JOIN stations s1 ON s1.code='BJP'
WHERE ts.train_no='K1' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
SELECT ts.id, s2.id, 2, '07:10'::time, NULL FROM train_services ts JOIN stations s2 ON s2.code='SHH'
WHERE ts.train_no='K1' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_segments(train_service_id,from_stop_seq,to_stop_seq,from_station_id,to_station_id,depart_time,arrive_time,duration)
SELECT ts.id,1,2,s1.id,s2.id,'19:00'::time,'07:10'::time,'12 hours 10 minutes'::interval
FROM train_services ts JOIN stations s1 ON s1.code='BJP' JOIN stations s2 ON s2.code='SHH'
WHERE ts.train_no='K1' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date AND NOT EXISTS(
  SELECT 1 FROM service_segments seg WHERE seg.train_service_id=ts.id AND seg.from_stop_seq=1 AND seg.to_stop_seq=2);
INSERT INTO segment_seat_inventory(train_service_id,segment_id,seat_type,total_seats,left_seats,price_cents)
SELECT ts.id, seg.id, x.seat_type, x.total, x.left_qty, x.price
FROM train_services ts
JOIN service_segments seg ON seg.train_service_id=ts.id AND seg.from_stop_seq=1 AND seg.to_stop_seq=2
JOIN (VALUES
  ('hardSeat'::seat_type_enum,900,500,12000),
  ('hardSleeper'::seat_type_enum,300,80,24000),
  ('softSleeper'::seat_type_enum,150,40,36000)
) AS x(seat_type,total,left_qty,price) ON true
WHERE ts.train_no='K1' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date
ON CONFLICT (train_service_id,segment_id,seat_type) DO UPDATE SET total_seats=EXCLUDED.total_seats,left_seats=EXCLUDED.left_seats,price_cents=EXCLUDED.price_cents,currency='CNY';
