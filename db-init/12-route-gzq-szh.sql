...
WHERE ts.train_no='G100' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
...
WHERE ts.train_no='G100' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date AND NOT EXISTS(
...
WHERE ts.train_no='G100' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date
...
WHERE ts.train_no='G101' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
...
FROM train_services ts JOIN stations s1 ON s1.code='GZQ' JOIN stations s2 ON s2.code='SZH'
WHERE ts.train_no='G101' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date AND NOT EXISTS(
  SELECT 1 FROM service_segments seg WHERE seg.train_service_id=ts.id AND seg.from_stop_seq=1 AND seg.to_stop_seq=2);
INSERT INTO segment_seat_inventory(train_service_id,segment_id,seat_type,total_seats,left_seats,price_cents)
SELECT ts.id, seg.id, x.seat_type, x.total, x.left_qty, x.price
FROM train_services ts
...
WHERE ts.train_no='G101' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date
ON CONFLICT (train_service_id,segment_id,seat_type) DO UPDATE SET total_seats=EXCLUDED.total_seats,left_seats=EXCLUDED.left_seats,price_cents=EXCLUDED.price_cents,currency='CNY';

-- G201 09:40-10:20（新增班次）
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
SELECT ts.id, s1.id, 1, NULL, '09:40'::time FROM train_services ts JOIN stations s1 ON s1.code='GZQ'
WHERE ts.train_no='G201' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date ON CONFLICT (train_service_id,stop_seq) DO NOTHING;
INSERT INTO service_stops(train_service_id,station_id,stop_seq,arrival_time,depart_time)
...
FROM train_services ts JOIN stations s1 ON s1.code='GZQ' JOIN stations s2 ON s2.code='SZH'
WHERE ts.train_no='G201' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date AND NOT EXISTS(
  SELECT 1 FROM service_segments seg WHERE seg.train_service_id=ts.id AND seg.from_stop_seq=1 AND seg.to_stop_seq=2);
INSERT INTO segment_seat_inventory(train_service_id,segment_id,seat_type,total_seats,left_seats,price_cents)
SELECT ts.id, seg.id, x.seat_type, x.total, x.left_qty, x.price
FROM train_services ts
...
WHERE ts.train_no='G201' AND ts.service_date BETWEEN current_date AND (current_date + INTERVAL '15 days')::date
ON CONFLICT (train_service_id,segment_id,seat_type) DO UPDATE SET total_seats=EXCLUDED.total_seats,left_seats=EXCLUDED.left_seats,price_cents=EXCLUDED.price_cents,currency='CNY';
