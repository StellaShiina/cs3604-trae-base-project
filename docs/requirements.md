...
## 2. 用户登录与注册（来源：requirement_lzc.md）
### 2.1 登录页面
- 支持“账号登录/扫码登录”切换。
...
## 3. 个人中心与乘车人管理（来源：requirement_lzc.md、requirements_hem.md）
### 3.1 个人中心
- 侧边栏：个人信息、乘车人管理、订单管理。
...
- 默认乘车人：用户成功注册后，系统自动创建一名与注册者信息一致的默认乘车人（姓名、证件类型、证件号码、手机号）；该默认乘车人不可编辑、不可删除。
- 表单字段校验：
  - 姓名（2-30字符，中文/英文）
  - 证件类型（下拉）与证件号码（按类型校验）
  - 手机号码（11位）
...
- Scenario: 查看/添加/编辑/删除/批量删除
  ...
  - 注册自动生成默认乘车人：
    - Given 用户注册成功
    - When 打开乘车人列表
    - Then 列表包含一名默认乘车人，且其姓名/证件/手机号与注册信息一致。
  - 默认乘车人保护：
    - Given 用户存在默认乘车人
    - When 尝试编辑或删除该乘车人
    - Then 系统返回 HTTP 403 Forbidden，且乘车人信息保持不变。

## 4. 车票查询与筛选（来源：requirements.czr.md）
### 4.1 页面结构
...
## 5. 车票预订流程（来源：requirements_hem.md）
...
## 6. 订单管理（来源：requirements_hem.md、requirement_lzc.md）
